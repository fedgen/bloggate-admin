...
def addCategory(request, format=None):
    response = Response()
    data = request.data
    category = Category.objects.filter(category_name=data['category_name']).first()
...
@api_view(['POST'])
@authenticate_admin
def editCategory(request, format=None):
    response = Response()
    data = request.data
...
@api_view(['POST'])
@authenticate_admin
def removeCategory(request, format=None):
    response = Response()
    data = request.data
    category = Category.objects.filter(id=data['category_id']).first()
...
@api_view(['GET'])
def listCategories(request, format=None):
    response = Response()
    categories = Category.objects.all()
    categories_list = [{'category_id': cat.id, 'category_name': cat.category_name, 'description': cat.description} for cat in categories]

    response.data = {
        'ok': True,
        'details': categories_list
    }
    response.status_code = 200
    return response
...
@api_view(['POST'])
@authenticate_admin
def addTag(request, format=None):
    response = Response()
    data = request.data
    tag = Tag.objects.filter(tag_name=data['tag_name'], category_id=data['category_id']).first()

    if tag is None:
        tag = Tag(tag_name=data['tag_name'], category_id=data['category_id'])
        tag.save()
        response.data = {
            'ok': True,
            'details': 'Tag added'
...
@api_view(['POST'])
@authenticate_admin
def editTag(request, format=None):
    response = Response()
    data = request.data
    tag = Tag.objects.filter(id=data['tag_id']).first()

    if tag:
        tag.tag_name = data.get('tag_name', tag.tag_name)
        tag.category_id = data.get('category_id', tag.category_id)
        tag.save()
        response.data = {
            'ok': True,
            'details': 'Tag updated'
...
@api_view(['POST'])
@authenticate_admin
def removeTag(request, format=None):
    response = Response()
    data = request.data
    tag = Tag.objects.filter(id=data['tag_id']).first()
...
@api_view(['GET'])
def listTags(request, format=None):
    response = Response()
    tags = Tag.objects.all()
    tags_list = [{'tag_id': tag.id, 'tag_name': tag.tag_name, 'category_id': tag.category_id} for tag in tags]
...
@api_view(['POST'])
@authenticate_admin
def addPartner(request, format=None):
    response = Response()
    data = request.data
    partner = Partner.objects.filter(partner_name=data['partner_name']).first()
...
@api_view(['POST'])
@authenticate_admin
def editPartner(request, format=None):
    response = Response()
    data = request.data
...
@api_view(['POST'])
@authenticate_admin
def removePartner(request, format=None):
    response = Response()
    data = request.data
    partner = Partner.objects.filter(id=data['partner_id']).first()
...
@api_view(['GET'])
def viewPartners(request, format=None):
    response = Response()
